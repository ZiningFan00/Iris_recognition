...
def loc_norm_extract(folder_path):
    ...
    X=[]
    y=[]
    for image_path in image_total_path[1:]:
...
if __name__=="__main__":
    ...
    f_train,LDA=match.FisherLinearDis(X_train,y_train)
    f_train=np.asarray(f_train)
    # LDA=np.asarray(y_train)
    np.savetxt("feature.csv", f_train, delimiter=",")
    # np.savetxt("LDA.csv", LDA, delimiter=",")


    # f_test_list,f_test_re_list,label_test,label_test_re,dist_test_list=test(folder_path,f_train,X_train,y_train,LDA)
    ...
    # dist_test_list=np.asarray(dist_test_list)
    np.savetxt( "clas_list.csv",clas_list, delimiter=",")
    np.savetxt("f_test_list.csv", f_test_list, delimiter=",")
    np.savetxt("f_test_re_list.csv", f_test_re_list, delimiter=",")
    np.savetxt( "label_test.csv",label_test, delimiter=",")
    np.savetxt( "label_test_re.csv",label_test_re, delimiter=",")
    ...
    # eva.evaluateIdentification(label_test_re,label_test,y_train)
